...
from download_qff_tsm_15m_data import (
    TAIPEI_TZ,
    normalize_ohlcv_frame,
    parse_taipei_timestamp,
    print_summary,
    validate_frame,
    write_csv,
)
...
# (exchange, symbol, output stem, intervals, n_bars per interval,
#  expected Taipei bar-start hours - warn only)
...
TV_DOWNLOADS = [
    ("TAIFEX", "CCF1!", "ccf1", ["5m", "15m"], {"5m": 50_000, "15m": 30_000},
     CCF_SESSION_HOURS),
    ("NYSE", "UMC", "nyse_umc", ["5m", "15m"], {"5m": 30_000, "15m": 15_000},
     UMC_SESSION_HOURS),
    ("FX_IDC", "USDTWD", "fxidc_usdtwd", ["5m", "15m", "1h"],
...
def main(argv: list[str]) -> int:
    ...
    tv = TvDatafeed()
    for exchange, symbol, stem, interval_keys, n_bars_map, hours in TV_DOWNLOADS:
        for interval_key in interval_keys:
            ...
            out_path = out_dir / f"{stem}_{interval_key}_taipei_tv.csv"
            write_csv(frame, out_path, f"{exchange}:{symbol}")
