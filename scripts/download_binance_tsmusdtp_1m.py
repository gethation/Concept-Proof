...
DEFAULT_OUTPUT_PATH = Path("data/processed/binance_tsmusdtp_1m_taipei.csv")
DEFAULT_SYMBOL = "TSM/USDT:USDT"
TIMEFRAME = "1m"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Download Binance USDT-M TSMUSDT perpetual 1m OHLCV via ccxt, "
            "using the same timestamp range as the TAIFEX QFF1 CSV."
        )
    )
...
def fetch_range(
    ...
    if symbol not in exchange.markets:
        matches = [
            ...
            if "TSM" in market_symbol or "TSM" in market.get("id", "")
        ]
        raise RuntimeError(f"Symbol {symbol} not found. TSM-like markets: {matches}")
...
def main(argv: list[str]) -> int:
    ...
    print(f"Using Binance USDT-M market: {args.symbol} ({market['id']})")
