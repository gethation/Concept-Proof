...
DEFAULT_SUMMARY_PATH = paths.run_dir("scratch") / "parameter_grid.csv"
DEFAULT_REPORT_PATH = paths.report("qff_tsm_parameter_grid")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--qff-ohlcv", type=Path, default=engine.DEFAULT_QFF_OHLCV_PATH)
    parser.add_argument("--tsm-ohlcv", type=Path, default=engine.DEFAULT_TSM_OHLCV_PATH)
    parser.add_argument(
        "--usdttwd-ohlcv", type=Path, default=engine.DEFAULT_USDTTWD_OHLCV_PATH
    )
    parser.add_argument(
        "--windows",
        type=parse_int_list,
    ...
    parser.add_argument("--tsm-fee-bps", type=float, default=engine.DEFAULT_TSM_FEE_BPS)
    parser.add_argument(
        "--qff-fee-per-contract-twd",
        type=float,
        default=engine.DEFAULT_QFF_FEE_PER_CONTRACT_TWD,
    )
    parser.add_argument(
        "--qff-fee-bps",
        type=float,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "backtest/engine.py --executable-displacement."
        ),
    )
    parser.add_argument("--qff-tax-rate", type=float, default=engine.DEFAULT_QFF_TAX_RATE)
    parser.add_argument(
        "--qff-contract-multiplier",
        type=float,
        default=engine.DEFAULT_QFF_CONTRACT_MULTIPLIER,
    )
    parser.add_argument(
        "--annual-trading-days",
        type=float,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "features/zscore.py --seed-spread-path); removes the "
...
def make_params(args: argparse.Namespace, entry_z: float, exit_z: float) -> engine.BacktestParams:
    ...
    return engine.params_from_args(args, entry_z=entry_z, exit_z=exit_z)
...
def sweep_cells(
    ...
    for entry_z in args.entry_z:
        for exit_z in args.exit_z:
            try:
                engine.validate_params(make_params(args, entry_z, exit_z))
...
def run_grid(args: argparse.Namespace) -> pd.DataFrame:
    ...
    for window in args.windows:
        ...
        backtest_frame = engine.add_entry_open_prices(
            zframe,
        ...
        for entry_z, exit_z in cells:
            ...
            result = engine.run_backtest(backtest_frame, params)
